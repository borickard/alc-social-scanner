...
- username: the @handle text shown on the video (usually bottom-left). Return just the handle text WITHOUT the leading "@". Null if not visible.
- display_name: the human-readable display name shown above or near the handle. If only a handle is visible, copy the handle text here too (still without the "@"). Null if neither is visible.
